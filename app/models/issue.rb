class Issue < ApplicationRecord
  ...
  scope :done_between_today_7days, -> { where("closed_on IS NOT NULL").where(closed_on: Date.today - 7..Date.today) }
  scope :done_between_7days_14days, -> { where("closed_on IS NOT NULL").where(closed_on: Date.today - 14..Date.today - 7) }
  scope :done_between_14days_21days, -> { where("closed_on IS NOT NULL").where(closed_on: Date.today - 21..Date.today - 14) }
